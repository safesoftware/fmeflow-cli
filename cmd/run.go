...
func newRunCmd() *cobra.Command {
  ...
  fmeflow run --repository Samples --workspace austinApartments.fmw --file Landmarks-edited.sqlite --wait`,
	...
	cmd.Flags().StringVar(&f.queue, "queue", "", "Queue of the job to submit. Equavalent to --tag (deprecated).")
	cmd.Flags().StringVar(&f.queue, "tag", "", "The queue (job routing tag) for the request.")
	cmd.Flags().IntVar(&f.maxJobRuntime, "max-job-runtime", -1, "Time (in seconds) elapsed for a running job before it's cancelled. The minimum value is 1 second, values less than 1 second are ignored. Equavalent to --time-until-canceled (deprecated).")
	...
	cmd.Flags().IntVar(&f.maxTimeInQueue, "max-time-in-queue", -1, "Time to live in the job queue (in seconds). Equavalent to --time-to-live (deprecated).")
