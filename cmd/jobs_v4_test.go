...
func TestJobsV4(t *testing.T) {
	...
	cases := []testCase{
		...
		{
			name:            "get jobs v4 succeeded and failed",
			statusCode:      http.StatusOK,
			args:            []string{"jobs", "--succeeded", "--failed"},
		...
		{
			name:           "get jobs v4 succeeded and failed json",
			statusCode:     http.StatusOK,
			args:           []string{"jobs", "--succeeded", "--failed", "--json"},
			body:           responseV4SuccessAndFailure,
			wantOutputJson: responseV4SuccessAndFailure,
			fmeflowBuild:   25300,
		},
		{
			name:            "get jobs v4 succeeded and failed no headers",
			statusCode:      http.StatusOK,
			args:            []string{"jobs", "--succeeded", "--failed", "--no-headers"},
			body:            responseV4SuccessAndFailure,
			wantOutputRegex: "^[\\s]*1[\\s]*387f74cd4e1f[\\s]*austinApartments.fmw[\\s]*success[\\s]*2[\\s]*10f259e906e5[\\s]*none2none.fmw[\\s]*failure[\\s]*$",
			fmeflowBuild:    25300,
		},
		{
			name:           "get jobs v4 succeeded and failed json output type",
			statusCode:     http.StatusOK,
			args:           []string{"jobs", "--succeeded", "--failed", "--output=json"},
			body:           responseV4SuccessAndFailure,
			wantOutputJson: responseV4SuccessAndFailure,
			fmeflowBuild:   25300,
		},
		...
		{
			name:            "get jobs v4 succeeded and failed custom columns",
			statusCode:      http.StatusOK,
			body:            responseV4SuccessAndFailure,
			args:            []string{"jobs", "--succeeded", "--failed", "--output", "custom-columns=CPU:.cpuTime,MEMORY:.peakMemoryUsage"},
