...
```
      ...
      --tag string                             The job routing tag for the request. For v3 API only.
      ...
      --time-until-canceled int                Time (in seconds) elapsed for a running job before it's cancelled. The minimum value is 1 second, values less than 1 second are ignored. For v3 API only. (default -1)
      --time-to-live int                       Time to live in the job queue (in seconds). For v3 API only. (default -1)
      ...
      --queue string                           Queue of the job to submit. For v4 API only.
      --max-job-runtime int                    Time (in seconds) elapsed for a running job before it's cancelled. The minimum value is 1 second, values less than 1 second are ignored. For v4 API only.
      --max-time-in-queue int                  Time to live in the job queue (in seconds). For v4 API only.
      --max-total-life-time int                Time to live including both time in the queue and run time (in seconds). The maximum value is 86400 and the minimum value is 1. For v4 API only.
